...
public final class Checksums {
    ...
    private static long computeChecksum(byte magic, byte attributes, long timestamp, ByteBuffer key, ByteBuffer value) {
        ...
        if (magic > RecordBatch.MAGIC_VALUE_V0)
            org.apache.kafka.common.utils.Checksums.updateLong(crc, timestamp);
        // update for the key
        if (key == null) {
            org.apache.kafka.common.utils.Checksums.updateInt(crc, -1);
        } else {
            int size = key.remaining();
            org.apache.kafka.common.utils.Checksums.updateInt(crc, size);
            org.apache.kafka.common.utils.Checksums.update(crc, key, size);
        }
        ...
        if (value == null) {
            ...
            org.apache.kafka.common.utils.Checksums.updateInt(crc, size);
            org.apache.kafka.common.utils.Checksums.update(crc, value, size);
        }
        return crc.getValue();
    }
}
