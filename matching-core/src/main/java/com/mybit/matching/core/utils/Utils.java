...
public final class Utils {
    ...
    /**
     * Read a UTF8 string from a byte buffer with prefix length. And update the position of the byte buffer
     *
     * @param buffer The buffer to read from
     * @return The UTF8 string
     */
    public static String utf8WithLen(ByteBuffer buffer) {
        int len = buffer.getInt();
        if (len <= 0) return "";

        String s = utf8(buffer, len);
        buffer.position(buffer.position() + len);
        return s;
    }

    public static void writeUtf8Str(ByteBuffer buffer, String s) {
        if (s == null || s.isEmpty()) {
            buffer.putInt(0);
            return;
        }
        byte[] bytes = Utils.utf8(s);
